...
class Testdumpshas2file(TestsWithData):
    def test_noretry(self):
        ...
        mock_fp = MagicMock()
        utils.dumpshas2file(mock_fp, commit, "a", "b", commit.status)
        utils.dumpshas2file(mock_fp, commit2, "a", "b", commit2.status)
        utils.dumpshas2file(mock_fp, commit3, "a", "b", commit3.status)
        expected = [
