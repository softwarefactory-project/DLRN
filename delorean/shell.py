    ...
    for commit in commits:
        ...
        for pkg in package_info["packages"]:
            ...
            if project == commit.project_name:
                if pkg["upstream"].startswith("git://git.openstack.org"):
                    commit_url = pkg["upstream"].split("/")[3:]
                    commit_url = "http://git.openstack.org/cgit/" + \
                                 "/".join(commit_url)
                    commit_url += "/commit/?id="
                if pkg["upstream"].startswith("git://github.com"):
                    commit_url = pkg["upstream"].split("/")[3:]
                    commit_url = "https://github.com/" + "/".join(commit_url)
                    commit_url += "/commit/"
