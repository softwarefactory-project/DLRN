...
def build(cp, package_info, commit, env_vars, dev_mode, use_public):
    ...
    yumrepodir = os.path.join(cp.get("DEFAULT", "distro"),
                              commit.getshardedcommitdir())
    ...
    current_repo_dir = os.path.join(datadir, cp.get("DEFAULT", "distro"),
                                    "current")
    os.symlink(os.path.relpath(yumrepodir_abs, os.path.join(datadir,
                               cp.get("DEFAULT", "distro"))),
               current_repo_dir + "_")
