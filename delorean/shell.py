...
def genreports(cp, package_info):
    ...
    for package in sorted(packages, cmp=lambda x,y: cmp(x['name'], y['name'])):
        name = package["name"]
        ...
        if commits.first().status == "SUCCESS":
            ...
            html.append("<td><i class='fa fa-thumbs-o-up pull-left' "
                        "style='color:green'></i><a href='%s/rpmbuild.log'>SUCCESS</a></td>"
            ...
            html.append("<td>%s</td>" % name)
            html.append("<td><i class='fa fa-thumbs-o-down pull-left' "
                        "style='color:red'></i><a href='%s/rpmbuild.log'>FAILED</a></td>"
                        % commits.first().getshardedcommitdir())
            ...
            if commits.count() == 0:
                ...
                html.append("<td><i class='fa fa-link pull-left' "
                            "style='color:#004153'></i>"
                            "<a href='%s/rpmbuild.log'>%s</a></td>"
                            % (commits.first().getshardedcommitdir(),
                               commits.first().commit_hash))
            if last_success_dt == 0:
                html.append("<td>Never</td>")
            else:
                html.append("<td>%d days</td>" % (datetime.now() - datetime.fromtimestamp(last_success_dt)).days)
