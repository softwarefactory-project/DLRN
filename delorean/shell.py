    ...
    for package in package_info["packages"]:
        ...
        if not options.package_name or package["name"] == options.package_name:
            ...
            if build_added is False:
                last_build = session.query(Commit).filter(Commit.project_name == project).order_by(desc(Commit.id)).first()
...
def rebuild():
    ...
    last_build = session.query(Commit).filter(Commit.project_name == project).order_by(desc(Commit.id)).limit(1).first()
    if not last_build:
