    ...
    try:
        ...
        if "master" in branch:
            ...
            if branch.startswith("rpm-"):
                # TODO general distro branch detection
                branch="rpm-master"
            else:
                branch="master"
...
def getinfo(cp, project, repo, distro, since, local, dev_mode, package):
    ...
    if dev_mode is False:
        distro_branch, distro_hash, dt_distro = refreshrepo(distro, distro_dir, distro_branch, local=local)
    else:
        distro_hash = "dev"
    ...
    for repo in repos:
        ...
        source_branch, _, _ = refreshrepo(repo, repo_dir, source_branch, local=local)
