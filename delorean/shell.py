...
def getmasterbranch(cp, package):
    if 'master-branch' in package:
        return package['master-branch']
    else:
        return cp.get("DEFAULT", "master")
...
def getinfo(cp, project, repo, spec, since, local, dev_mode, package):
    ...
    master_branch = getmasterbranch(cp, package)
