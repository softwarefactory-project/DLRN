...
class CoprBuildDriver(BuildRPMDriver):
    ...
    def build_package(self, **kwargs):
        ...
        copr_id = self.config_options.copr_id
        ...
        try:
            ...
            with open("%s/coprbuild.log" % output_dir, 'a') as self.copr_fp:
                try:
            ...
            for line in log_content:
                m = re.search("^Created build: (\d+)$", line)
                if m:
            ...
            run_cmd.extend(
                [self.exe_name,
                 'download-build', '-d', '%s/%s' % (output_dir, build_id),
                 build_id])

            with open("%s/coprdownload.log" % output_dir, 'a') as self.copr_fp:
                try:
                    sh.env(run_cmd, _err=self._process_copr_output,
                           _out=self._process_copr_output,
