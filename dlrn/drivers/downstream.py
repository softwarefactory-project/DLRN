...
class DownstreamInfoDriver(PkgInfoDriver):
    ...
    def getpackages(self, **kwargs):
        ...
        # XXX should this also be filtered to only contain packages
        # from the remote versions.csv too?
        return self.packages

    def getversions(self):
        """
        Fetch 'versions.csv' from versions_url config option
        and return the contained data as a dict with package
        name as a key.
        """
        ...
        content = map(lambda x: x.decode('utf8'), r.readlines())
    ...
    def getinfo(self, **kwargs):
        ...
        if dev_mode is False:
            try:
                ...
                distro_hash = versions[pacakge['name']][1]
            except Exception:
        ...
        for repo in repos:
            ...
            for line in lines:
                dt = str(line).strip().strip("'").split(" ")[0]
                commit_hash = versions[package['name']][3]
