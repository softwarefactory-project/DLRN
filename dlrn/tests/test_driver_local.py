...
class TestDriverLocal(base.TestCase):
    ...
    def test_getinfo(self, sh_mock):
        ...
        info = driver.getinfo(
            package=package, project="hello-world", dev_mode=False)
    ...
    def test_getpackages(self):
        ...
        open(os.path.join(distgit_dir, 'hello.spec'), 'w').write(
            'Version: 1.0.0'
        )
        # We to call DLRN from the local distgit directory
        os.chdir(distgit_dir)
        driver = LocalDriver(cfg_options=self.config)
        packages = driver.getpackages()
