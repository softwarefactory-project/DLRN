...
class TestUser(base.TestCase):
    ...
    @mock.patch('sys.exit')
    @mock.patch('dlrn.user.getSession')
    def test_user_create_duplicate(self, db_mock, ex_mock):
        db_mock.return_value = self.session
        testargs = ["dlrn-user", "create", "--username",
                    "user", "--password", "password"]
        with mock.patch.object(sys, 'argv', testargs):
    ...
    @mock.patch('sys.exit')
    @mock.patch('dlrn.user.getSession')
    def test_user_update_non_existing(self, db_mock, ex_mock):
        db_mock.return_value = self.session
    ...
    @mock.patch('sys.exit')
    @mock.patch('dlrn.user.getSession')
    def test_user_delete_non_existing(self, db_mock, ex_mock):
        db_mock.return_value = self.session
