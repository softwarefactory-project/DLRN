...
@mock.patch('dlrn.shell.post_build')
@mock.patch('dlrn.shell.getSession', side_effect=mocked_session)
@mock.patch('urllib2.urlopen', side_effect=mocked_urlopen)
class TestRemote(base.TestCase):
    def test_remote(self, url_mock, db_mock, build_mock):
        testargs = ["dlrn-remote", "--config-file", "projects.ini",
                    "--remote-yaml", "http://example.com/1/commit.yaml",
                    "--repo-url", "http://example.com/1/commit.yaml"]
