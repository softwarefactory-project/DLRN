...
class TestCIVoteAggregationQuery(DLRNAPIGraphQLTestCase):
    def test_basic_query(self, db_mock):
        response = self.app.get('/api/graphql?query={ civotesAgg { id } }')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(len(data['data']['civotesAgg']), 3)

    def test_filtered_query(self, db_mock):
        query = """
            query {
                civotesAgg(refHash: "12345678")
                {
                    id
                }
            }
        ...
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(len(data['data']['civotesAgg']), 2)

    def test_badfiltered_query(self, db_mock):
        query = """
            query {
                civotesAgg(commit_id: "TextnotInt")
                {
                    id
                }
            }
    ...
    def test_get_multiple_fields(self, db_mock):
        ...
        data = json.loads(response.data)
        self.assertEqual(len(data['data']['civotesAgg']), 2)
        self.assertEqual(data['data']['civotesAgg'][0]['refHash'], '12345678')
        self.assertEqual(data['data']['civotesAgg'][0]['ciName'], 'phase2-ci')
        self.assertEqual(data['data']['civotesAgg'][0]['ciUrl'],
                         'http://dummyci.example.com/phase2-ci')
        self.assertEqual(data['data']['civotesAgg'][0]['ciVote'], True)
        self.assertEqual(data['data']['civotesAgg'][0]['ciInProgress'], False)
        self.assertEqual(data['data']['civotesAgg'][0]['timestamp'], 1441635095)
        self.assertEqual(data['data']['civotesAgg'][0]['notes'], '')
        assert 'user' not in data['data']['civotesAgg'][0]
