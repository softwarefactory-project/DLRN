...
def getVote(session, timestamp, success, link_name):
    ...
    if vote is None:
        if link_name is not None:
            # No votes found for link_name, let's try to find one for
            # consistent
...
@app.route('/api/get_last_tested_repo', methods=['GET'])
def get_last_tested_repo_GET():
    ...
    if request.headers['Content-Type'] == 'application/json':
        ...
        try:
            success = bool(strtobool(request.json['success']))
            # success = False
...
@app.route('/api/get_last_tested_repo', methods=['POST'])
@auth.login_required
def get_last_tested_repo_POST():
    # timestamp: Timestamp in secs since the epoch used as base for the search
    # success(optional): find repos with a successful/unsuccessful vote
    # link_name(optional); name of the CI that sent the vote
    if request.headers['Content-Type'] == 'application/json':
        try:
            ...
            raise InvalidUsage('Missing parameters', status_code=400)
        try:
            my_link_name = request.json['reporting_link_name']
        except KeyError:
            raise InvalidUsage('Missing parameters', status_code=400)
        try:
            success = bool(strtobool(request.json['success']))
