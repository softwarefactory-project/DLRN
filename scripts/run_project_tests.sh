...
if type -p zuul-cloner; then
    ...
    if [ -n "$RDO_ID" ]; then
        PROJECT_DISTRO_ESCAPED=$(sed 's$/$%2F$' <<< $PROJECT_DISTRO)
        JSON=$(curl -s -L $GIT_BASE_URL/changes/${PROJECT_DISTRO_ESCAPED}~${PROJECT_DISTRO_BRANCH}~${RDO_ID}/revisions/current/review|sed 1d)
