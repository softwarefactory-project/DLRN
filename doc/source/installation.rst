...
* ``release_numbering`` defines the algorithm used by DLRN to assign release
  ...
  * ``legacy`` if the old method is used: 0.<date>.<hash>
  * ``new`` if the new method is used: 0.1.<date>.<hash>. This new method
    provides better compatibility with the Fedora packaging guidelines.
