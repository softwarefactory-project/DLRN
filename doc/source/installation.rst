...
.. code-block:: ini
    ...
    replace_list=^foo/bar,string1/string2
...
* ``replace_list``, when ``use_upstream_spec`` is set to True, will perform some
  ...
  has special requirements, such as building without documentation. in that case,
  a regular expresion like the following could be used:

.. code-block:: ini
    replace_list=^%global with_doc.+/%global with_doc 0
