...
* ``custom_preprocess``, if set, defines a custom program or script to be called
  as part of the pre-process step. After the distgit is cloned, and before the
  source RPM is built, the ``pkginfo`` drivers run a pre-process step where some
  actions are taken on the repository, such as Jinja2 template processing. In
  addition to this per-driver step, a custom pre-process step can be specified.
  The external program will be executed with certain environment variables set:
